...
**Version:** 1.1 · **Date:** September 25, 2026 · **Status:** M1/W1, N1.1–N1.3, M2, U1, G1.1, G1.2 and the Ariel-accepted M3 quest slice are delivered to `main`. G1.3 and the ad-hoc quest UI correction pass local release checks; Ariel authorized the current candidate's source push and Pages deployment. M3.5 authoring reports follow G1.3 before M4. See STATUS for verification and publication boundaries.
...
| M3 missing-lens and archive-ledger quests | Locally accepted and source pushed as `6c17550`; generic dialogue/quest/item hand-in, Journal, lit Gallery and v1 migration passed 356 unit/content tests and 146 browser scenarios | Decide Pages release separately |
| G1.3 independent locks | Workshop supply room, retained Workshop key, independent persistent west door, and safer Workshop/Gallery arrivals pass local checks; release authorized | Record the source push and Pages run; collect specific manual supply-room feedback separately |
...
**Ad-hoc dialogue follow-up:** Mara's earlier conditional plaque response was incorporated into the M3 dialogue graph. The accepted and pushed M3 source adds explicit acceptance, item hand-in, completion and save migration; see [STATUS](STATUS.md) for the checks and release boundary.
...
**Ad-hoc quest UI correction:** Ariel reported lost keyboard control after completion and confusing close behavior in the completed-quest Journal. The local correction restores focus to the live dialogue control after a choice is replaced, closes the Journal directly to exploration, and starts the next Inventory opening at Inventory. Ariel reports that the correction works and authorized release of the current candidate. M3.5 order is unchanged. See [STATUS](STATUS.md) for verification and delivery status.
...
- [x] **G1.3 — Second independent lock and usability proof.** A Workshop key opens only its persistent west supply-room door, while the brass key remains exclusive to the Gallery Storeroom. The playable return route and revised Workshop/Gallery arrivals have set-back spawns. The final full Chromium suite passed 148/148 after the quest UI correction, including locked/unlocked visuals, denial without a destination fetch, key isolation, failed-load recovery, save compatibility, held input, 40-transfer ownership, unconditional exits and occupied gate behavior. Existing returns and the switch gate were inspected; STATUS records the findings. Ariel authorized release of the current candidate; specific manual supply-room feedback remains separate.
...
**Release the verified G1.3 and quest UI candidate, then complete M3.5.** Ariel accepted the corrected M3 quests; their reviewed source reached `main` as `6c17550` without a Pages dispatch. The Workshop supply room uses its own key and door marker, and the Workshop/Gallery arrival spawns were set back after inspection. The current candidate passed 148/148 Chromium scenarios and Ariel authorized its source push and Pages deployment. Record the release results and specific hosted feedback separately. Complete M3.5 authoring reports before M4.
