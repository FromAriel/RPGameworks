...
**Ad-hoc character-art research (documented, not implemented):** The ignored Cute Fantasy player pack has a [sprite-sheet layout schema, populated map, and reconstruction guide](asset-analysis/README.md). It identifies verified layer order, row offsets, and split tool depth; Ariel's 56-row map now supplies animation names, directions, and frame counts even though the Aseprite file has no animation tags. The last two body cells of each `fish_cast` row are exact duplicates, but the rod overlay differs by four pixels; Ariel chose nine frames for all three cast directions. Side mirroring, playback timing, anchoring, and licensing approval remain open. No third-party art enters the production bundle from this research packet.

**Character-art implementation branch (CA1 synthetic foundations delivered, reconciled, pending review):** [CHARACTER-ASSET-INTEGRATION](CHARACTER-ASSET-INTEGRATION.md) is the detailed CA1–CA4 sub-plan for a versioned binary player pack, layered animation, lifecycle checks and artifact review. CA1's synthetic-pixel slice is delivered and locally verified (pack format [decision 0003](decisions/0003-character-pack-v1.md), deterministic encoder, strict reader, selection validator, committed anti-drift golden fixture, 27 unit tests — see [STATUS](STATUS.md)), Ariel's four format-review findings are reconciled, including the digest-abort and existing-pack preservation review follow-up, and her starter appearance is recorded in [starter-appearance.json](asset-analysis/starter-appearance.json). No character pixels are added, the runtime is unwired, and Ariel's re-review of the reconciled format remains before the real-pixel gate. G1.2 travel is still the next gameplay packet.
