...
export const SAVE_FORMAT='rpgameworks-save';export const SAVE_SCHEMA_VERSION=1;export const MAX_IMPORT_BYTES=256*1024;
...
export interface SaveEnvelopeV1{readonly format:typeof SAVE_FORMAT;readonly schemaVersion:1;readonly gameId:string;readonly saveCompatibilityVersion:number;readonly slotId:SaveSlotId;readonly storageRevision:number;readonly savedAt:string;readonly checkpoint:SaveCheckpoint;readonly session:SessionData}
export interface StateIndex{readonly schemaVersion:1;readonly gameId:string;readonly saveCompatibilityVersion:number;readonly maps:readonly{readonly id:string;readonly name:string;readonly width:number;readonly height:number;readonly spawns:readonly string[]}[];readonly itemIds:readonly string[];readonly factIds:readonly string[];readonly placementIds:readonly string[]}
...
export function validateStateIndex(value:unknown):StateIndex{
  if(!plain(value)||!exact(value,['schemaVersion','gameId','saveCompatibilityVersion','maps','itemIds','factIds','placementIds'])||value.schemaVersion!==1||typeof value.gameId!=='string'||!Number.isSafeInteger(value.saveCompatibilityVersion)||(value.saveCompatibilityVersion as number)<1||!Array.isArray(value.maps)||!Array.isArray(value.itemIds)||!Array.isArray(value.factIds)||!Array.isArray(value.placementIds))throw new Error('Invalid save state index');
  ...
  for(const ids of [value.itemIds,value.factIds,value.placementIds])if(!ids.every(id=>typeof id==='string')||new Set(ids).size!==ids.length)throw new Error('Invalid save state index identifiers');
...
export function validateSaveEnvelope(value:unknown,context:SaveContext):SaveEnvelopeV1{
  ...
  if(value.format!==SAVE_FORMAT||value.schemaVersion!==SAVE_SCHEMA_VERSION)throw new Error('Unsupported save format or version');
  if(value.gameId!==context.gameId)throw new Error('Save belongs to a different game');
  if(value.saveCompatibilityVersion!==context.saveCompatibilityVersion)throw new Error('Save is not compatible with this content version');
  ...
  const session=value.session;if(!plain(session)||!exact(session,['inventory','facts','placements'])||!plain(session.inventory)||!plain(session.facts)||!plain(session.placements))throw new Error('Saved session is invalid');
...
export function createEnvelope(slotId:SaveSlotId,storageRevision:number,context:SaveContext,checkpoint:SaveCheckpoint,session:SessionState,now=new Date()):SaveEnvelopeV1{
  return validateSaveEnvelope({format:SAVE_FORMAT,schemaVersion:1,gameId:context.gameId,saveCompatibilityVersion:context.saveCompatibilityVersion,slotId,storageRevision,savedAt:now.toISOString(),checkpoint,session:structuredClone(session.data())},context);
}
