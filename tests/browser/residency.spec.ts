...
test('test-only twelve-map residency tour keeps one room, one binding, and shared textures bounded',async({page})=>{
  ...
  const final=await snapshot(page);expect(final.mapId).toBe(mapId(0));expect(final.activeScenes).toBe(1);expect(final.loadedMaps).toBe(1);expect(final.sessionSubscribers).toBe(1);expect(final.activeObjectBindings).toBe(1);expect(final.heroArt).toBe('layered');expect(final.textureCount).toBe(baseline.textureCount);expect(final.displayObjects).toBe(baseline.displayObjects);expect(final.pendingSaveOperations).toBe(0);
});
