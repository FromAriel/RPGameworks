...
Open the [GitHub Pages playtest](https://fromariel.github.io/RPGameworks/) in a browser; players do not need npm. The latest verified build-only deployment serves game commit `fb17a1c0a5b2b479eb748f96137e157b02f612ee` from [manual run 35685752229](https://github.com/FromAriel/RPGameworks/actions/runs/35685752229), including the edited window texture. The build ID shown in Debug/Settings identifies the version actually served, which may lag behind `main` until the next manual release.

The repository's Pages source is **GitHub Actions**. A maintainer releases reviewed `main` by running `gh workflow run pages.yml --repo FromAriel/RPGameworks --ref main`, then waits for the build and deploy jobs to succeed and checks the live URL and build ID. [The dedicated workflow](.github/workflows/pages.yml) only installs locked dependencies, builds for `/RPGameworks/`, and uploads `dist/`; it does **not** run unit tests, browser tests, benchmarks, or the UI gallery. Run desired checks locally before approving a release. Ordinary pushes do not deploy, and the separate three-platform Foundation checks workflow is also manual-only. Do not describe a new commit as deployed until its own Pages run and live URL are verified.
...
The manually dispatched Foundation checks workflow checks Linux with Node 22.16.0 and Node 24.15.0, plus Windows with Node 24.15.0. Node 24 jobs use npm 11.6.2. Each matrix leg retains a `browser-build-<label>` artifact and `browser-evidence-<label>` report. Those artifacts are build/test outputs, not a deployed website. The workflow has read-only repository permissions and no push or pull-request trigger.

The first Pages deployment has [historical exact-commit hosted evidence](https://github.com/FromAriel/RPGameworks/actions/runs/35676907503) from its initial, test-heavy workflow. The workflow was subsequently made build-only to avoid charging a full test campaign on every manual publish. This does not substitute for the separate three-platform CI matrix, which has not been rerun for that commit. See [current status](docs/STATUS.md) for the distinct local, hosted, deployment, and playtest boundaries.
...
Edit the canonical records in `content/games/demo/maps/`, and register maps in `content/games/demo/game.json`. Run `npm run validate` for schema and cross-reference checks plus a concise authoring report. Broken references fail validation; unused definitions and structural map reachability are advisory because they may be intentional and do not prove gameplay solvability. For a deterministic, source-relative JSON inventory of definitions, referrers, and warnings, run `npm run validate -- --report-json .tmp/authoring-report.json`. This explicit report option is only available with validation and is kept out of source, public assets, generated content, and `dist/`. `npm run content` rebuilds map payloads; refresh the development page afterward. Development startup and production builds run this automatically. Live file watching for content compilation is not implemented yet.
