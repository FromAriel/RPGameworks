...
## Final-delivery test synchronization

After the isolated candidate passed all three environments, final main revision `a1fe12da17fe2ac69cd209dc080660230d46e053` received [run 35336736792](https://github.com/FromAriel/RPGameworks/actions/runs/35336736792). Linux/Node 24 passed, while Linux/Node 22 passed 55 of 56 browser scenarios and exposed a synchronization race in the literal-text/reduced-effects case. All 178 unit tests and source/build checks passed there. This run is not recorded as an all-platform pass.

That test sent Escape and immediately pressed a movement key, before the scene's next update consumed the queued close action and restored viewport focus. The movement was correctly ignored while dialogue owned input. The test now waits for the dialog to close and for the stage to regain focus. Its shared single-step helper also observes exploration mode and an idle actor before sending input, and always releases the key in a finally block. It does not force focus, change game state, add retries, remove assertions, or suppress errors. An explicit return-tile assertion strengthens the reduced-effects case.

This follow-up changes test synchronization and this record only; gameplay code and test counts (178 unit / 56 browser) remain unchanged. Its own final CI run must pass before delivery is reported as verified. See [INTERACTIONS.md](INTERACTIONS.md) for the implemented behavior and earlier candidate evidence.
