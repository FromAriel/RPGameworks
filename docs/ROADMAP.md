...
**Character-art implementation branch (planned, not started):** [CHARACTER-ASSET-INTEGRATION](CHARACTER-ASSET-INTEGRATION.md) is the detailed CA1–CA4 sub-plan for a versioned binary player pack, layered animation, lifecycle checks and artifact review. It branches from the current post-G1.1c codebase without closing G1.1 or replacing G1.2 travel as the main gameplay packet. No character pixels are added by this planning step.
